...
### How do I update DClaw Space?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp space --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Space without Kubernetes?

Yes, for local development:

```bash
cd dclaw-space/frontend && npm run dev
cd dclaw-space/backend && uvicorn dclaw_space.main:app --reload
```
