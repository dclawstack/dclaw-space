...
# Check events
kubectl get events -n dclaw-space --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-space deployment/dclaw-space-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-space-db-credentials -n dclaw-space
   ```
